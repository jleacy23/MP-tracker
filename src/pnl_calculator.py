...
class Pnl_Calculator:
    ...
    def update_csv_with_profit_change(self, csv_file, balance, net_deposits):
    ...
    def report_latest_profit_change_and_commission(self, commission_ratio = 0.5):
        profit_change = 0
    ...
    def settle_profit(self, csv_file):
        if os.path.exists(f"{self.directory}/{csv_file}"):
            df = pd.read_csv(f"{self.directory}/{csv_file}")
            if not df.empty:
                df["settled"] = True
